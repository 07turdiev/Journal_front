    ...
    <div class="page-content">
      <div class="container">
        ...
        <div v-else-if="error" class="error-state">
          ...
          <button @click="loadEditorialBoardData" class="retry-btn">{{ $t('footer.retry') }}</button>
        </div>
        
        <div v-else class="members-list">
          <div v-if="boardMembers.length === 0" class="no-results">{{ $t('staff.no_members') }}</div>
          <div v-for="member in boardMembers" :key="member.id" class="member-item">
              ...
              <div class="member-info">
                ...
                <div class="member-contacts">
                  <div class="contact-item">
                    ...
                    <span>{{ member.phone || $t('staff.no_phone') }}</span>
                  </div>
                  <div class="contact-item">
                    <svg width="18" height="18" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span>{{ member.email || $t('staff.no_email') }}</span>
                  </div>
                </div>
              </div>
            </div>
...
const loadEditorialBoardData = async () => {
  try {
    const response = await fetchData('/tahrir-hayatis', { populate: 'Rasmi' });
    
    if (response.data) {
      boardMembers.value = response.data.map(member => ({
        id: member.id,
        name: member.Ismi,
    ...
    console.error('Editorial board data yuklanmadi:', err);
  }
};
